...
import torch
import copy
from work_RL_brain_down import DQN
import time
import os
import sys
...
Count_Departures_path = os.path.join("./test_data/", str(busline), f"Count_Departures{busline}_{direction1}_{omega_factor_up}.txt")

# 从文件中读取 count_departures 的值
with open(Count_Departures_path , "r") as file:
    count_departures_up = int(file.read())
# 在这里使用 count_departures 变量
# print("Count Departures:", count_departures)

max_episode = 500  # 最多训练代数
omega = 1 / omega_factor  # 两个用于计算reward的系数
beta = 0.2

first_minute_th = (int(first_time[:-3]) - int(trf_con.iloc[0, 0])) * 60 + (
            int(first_time[-2:]) - int(trf_con.iloc[0, 1]))
...
start_minute_th = (int(start_time[:-3]) - int(trf_con.iloc[0, 0])) * 60 + (
            int(start_time[-2:]) - int(trf_con.iloc[0, 1]))
...
class Station:
    def __init__(self, station_number=station_num, all_passenger_info_path=passenger_info_path):
        ...
        for i in range(self.station_number):  # 所有站点，每分钟上车乘客
            self.all_station_all_minute_passenger.append(self.all_passenger_info_dataframe[
                                                             self.all_passenger_info_dataframe[
                                                                 "Boarding station"] == i])  # 每站，所有分钟的
    ...
    def reset(self):
        ...
        for i in range(self.station_number):  # 所有站点，每分钟上车乘客
            self.all_station_all_minute_passenger.append(
                self.all_passenger_info_dataframe[
                    self.all_passenger_info_dataframe["Boarding station"] == i])  # 每站，所有分钟的
        for i in range(self.station_number):  # 第一分钟，所有站点的乘客
            self.init_frist_minute_passenger.append(self.all_station_all_minute_passenger[i][
                                                        self.all_station_all_minute_passenger[i][
                                                            "Arrival time"] <= first_minute_th])
...
class Bus:
    def __init__(self, pn_on_max=pn_on_max, station_number=station_num, start_time=right_minute_th):
        ...
        self.pn_on = pd.DataFrame(
            columns=["Label", "Boarding time", "Boarding station", "Alighting station", "Arrival time"])  # 车上人员
        ...
        self.Passenger_on_board_leaving_station = pd.DataFrame(
        ...
        for i in range(1, station_number):
            ...
            self.Estimated_arrival_time[i] = right_minute_th + sum(
                trf_con.iloc[(right_minute_th // 15), 6:6 + i]) - self.pass_minute

        self.To_be_process_all_station = []
        for i in range(0, station_number):  # 向这辆车的中加入所有乘客的信息标签
            self.To_be_process_all_station.append(pd.DataFrame(
                columns=["Label", "Boarding time", "Boarding station", "Alighting station", "Arrival time"]))
        ...
        for i in range(station.station_number):
            if self.there_label == 1 and self.position[i] == 1:
                ...
                self.pn_on = self.pn_on.drop_duplicates(
                    subset=["Label", "Boarding time", "Boarding station", "Alighting station"], keep=False)
                ...
                if len(station.right_minute_passenger[i]) + len(self.pn_on) <= self.pn_on_max:
                    ...
                    All_passenger_wait_time = All_passenger_wait_time + right_minute_th * len(
                        station.right_minute_passenger[i]) - np.sum(station.right_minute_passenger[i].iloc[:, 4])
                    station.right_minute_passenger[i] = pd.DataFrame(
                        columns=["Label", "Boarding time", "Boarding station", "Alighting station", "Arrival time"])
                    ...
                    All_passenger_wait_time = All_passenger_wait_time + right_minute_th * append_mark - np.sum(
                        station.right_minute_passenger[i].iloc[:append_mark, 4])
...
class BUS_LINE_SYSTEM:
    ...
    def Departure(self, ):
        self.bus_online.append(
            Bus(pn_on_max=pn_on_max, station_number=station_num, start_time=right_minute_th))  # 在列表中加入一辆巴士
        ...
        if len(self.bus_online) == 0:
            ...
            for i in range(self.station_number):
                self.bus_online[0].To_be_process_all_station[i] = pd.concat([self.station.right_minute_passenger[i],
                                                                             self.station.Estimated_psger_procs(i,
                                                                                                                right_minute_th,
                                                                                                                self.bus_online[
                                                                                                                    0].Estimated_arrival_time[
                                                                                                                    i])])  # 计算得到每一个站点将要上车的乘客是哪些

                self.bus_online[0].pn_on = self.bus_online[0].pn_on.append(
                    self.bus_online[0].pn_on[self.bus_online[0].pn_on["Alighting station"] == i])  # 计算要下车的乘客

                self.bus_online[0].pn_on = self.bus_online[0].pn_on.drop_duplicates(
                    subset=["Label", "Boarding time", "Boarding station", "Alighting station"],
                    keep=False)  # 在列表中删掉下车的乘客

                if len(self.bus_online[0].To_be_process_all_station[i]) + len(self.bus_online[0].pn_on) <= \
                        self.bus_online[0].pn_on_max:  # 若没有出现滞留
                    self.bus_online[0].pn_on = pd.concat(
                        [self.bus_online[0].pn_on, self.bus_online[0].To_be_process_all_station[i]])  # 加入将要处理的站点信息
                    self.bus_online[0].Left_after_pass_the_station[i] = pd.DataFrame(
                        columns=["Label", "Boarding time", "Boarding station", "Alighting station",
                                 "Arrival time"])  # 创建一个表示滞留乘客的
                    self.All_psger_wait_time = self.All_psger_wait_time + self.bus_online[0].Estimated_arrival_time[
                        i] * len(self.bus_online[0].To_be_process_all_station[i]) - np.sum(
                        self.bus_online[0].To_be_process_all_station[i].iloc[:, 4])  # 计算所有乘客所需的等待时间
                    self.All_psger_wait_time_depart_once = self.All_psger_wait_time_depart_once + \
                                                           self.bus_online[0].Estimated_arrival_time[i] * len(
                        self.bus_online[0].To_be_process_all_station[i]) - np.sum(
                        self.bus_online[0].To_be_process_all_station[i].iloc[:, 4])
                    ...
                    self.bus_online[0].pn_on.append(
                        self.bus_online[0].To_be_process_all_station[i].iloc[:append_mark, :])
                    self.bus_online[0].Left_after_pass_the_station[i] = self.bus_online[0].To_be_process_all_station[
                                                                            i].iloc[append_mark:, :]
                    self.All_psger_wait_time = self.All_psger_wait_time + self.bus_online[0].Estimated_arrival_time[
                        i] * append_mark - np.sum(self.bus_online[0].To_be_process_all_station[i].iloc[:append_mark, 4])
                    self.All_psger_wait_time_depart_once = self.All_psger_wait_time_depart_once + \
                                                           self.bus_online[0].Estimated_arrival_time[
                                                               i] * append_mark - np.sum(
                        self.bus_online[0].To_be_process_all_station[i].iloc[:append_mark, 4])
                    self.All_cap_uesd = self.All_cap_uesd + len(self.bus_online[-1].pn_on)
                    self.All_cap_uesd_depart_once = self.All_cap_uesd_depart_once + len(self.bus_online[-1].pn_on)
                    self.bus_online[-1].left_every_station_pn_on[i] = len(self.bus_online[-1].pn_on)
                    self.bus_online[-1].cant_taken_once = self.bus_online[-1].cant_taken_once + len(
                        self.bus_online[0].To_be_process_all_station[i]) - append_mark

        elif len(self.bus_online) > 1:
            for i in range(self.station_number):
                self.bus_online[-1].To_be_process_all_station[i] = pd.concat(
                    [self.bus_online[-2].Left_after_pass_the_station[i],
                     self.station.Estimated_psger_procs(i, self.bus_online[-2].Estimated_arrival_time[i],
                                                        self.bus_online[-1].Estimated_arrival_time[i])])

                self.bus_online[-1].pn_on = self.bus_online[-1].pn_on.append(
                    self.bus_online[-1].pn_on[self.bus_online[-1].pn_on["Alighting station"] == i])
                self.bus_online[-1].pn_on = self.bus_online[-1].pn_on.drop_duplicates(
                    subset=["Label", "Boarding time", "Boarding station", "Alighting station"], keep=False)

                if len(self.bus_online[-1].To_be_process_all_station[i]) + len(self.bus_online[-1].pn_on) <= \
                        self.bus_online[-1].pn_on_max:

                    self.bus_online[-1].pn_on = pd.concat(
                        [self.bus_online[-1].pn_on, self.bus_online[-1].To_be_process_all_station[i]])
                    self.bus_online[-1].Left_after_pass_the_station[i] = pd.DataFrame(
                        columns=["Label", "Boarding time", "Boarding station", "Alighting station", "Arrival time"])
                    self.All_psger_wait_time = self.All_psger_wait_time + self.bus_online[-1].Estimated_arrival_time[
                        i] * len(self.bus_online[-1].To_be_process_all_station[i]) - np.sum(
                        self.bus_online[-1].To_be_process_all_station[i].iloc[:, 4])
                    self.All_cap_uesd = self.All_cap_uesd + len(self.bus_online[-1].pn_on)
                    self.All_psger_wait_time_depart_once = self.All_psger_wait_time_depart_once + \
                                                           self.bus_online[-1].Estimated_arrival_time[
                                                               i] * len(
                        self.bus_online[-1].To_be_process_all_station[i]) - np.sum(
                        self.bus_online[-1].To_be_process_all_station[i].iloc[:, 4])
                    self.All_cap_uesd_depart_once = self.All_cap_uesd_depart_once + len(self.bus_online[-1].pn_on)
                    ...
                    self.bus_online[-1].pn_on.append(
                        self.bus_online[-1].To_be_process_all_station[i].iloc[:append_mark, :])
                    self.bus_online[-1].Left_after_pass_the_station[i] = self.bus_online[-1].To_be_process_all_station[
                                                                             i].iloc[append_mark:, :]
                    self.All_psger_wait_time = self.All_psger_wait_time + self.bus_online[-1].Estimated_arrival_time[
                        i] * append_mark - np.sum(
    ...
    def Action(self, Departure_factor=0, min_Interval=min_Interval, max_Interval=max_Interval):
        ...
        # 发车
        if (
                start_minute_th <= right_minute_th <= end_minute_th) and self.Interval == ideal_interval - 1 and avg_flag == 1:
            self.Departure()
            self.Interval = 0
            departure_label = 1

        elif (
                right_minute_th == first_minute_th or right_minute_th == last_minute_th or self.Interval == max_Interval) and right_minute_th < last_minute_th + 1:  #
            self.Departure()
    ...
    def get_state(self):
        ...
        Passenger_num_on_board_leaving_station = pd.DataFrame(
            columns=["Label", "Boarding time", "Boarding station", "Alighting station", "Arrival time"])
        ...
        for i in range(1, self.station_number - 1):
            right_deapart_estimated_arrival_time[i] = right_minute_th + sum(
                trf_con.iloc[(right_minute_th // 15), 6:6 + i])  # 通过当前时间点和历史的交通状况计算得到预计到站的时间
        ...
        for i in range(self.station_number):
            ...
            Passenger_tobe_proc_per_station.append(pd.concat([self.bus_online[-1].Left_after_pass_the_station[i],
                                                              self.station.Estimated_psger_procs(i, self.bus_online[
                                                                  -1].Estimated_arrival_time[i],
                                                                                                 right_deapart_estimated_arrival_time[
                                                                                                     i])]))  # 包含了滞留乘客信息和预计到达站点的乘客的信息，即包含了这个站点中需要被处理的乘客的信息
            ...
            Passenger_num_on_board_leaving_station = Passenger_num_on_board_leaving_station.append(
                Passenger_num_on_board_leaving_station[
                    Passenger_num_on_board_leaving_station["Alighting station"] == i])  # 计算第i站下车的乘客
            Passenger_num_on_board_leaving_station = Passenger_num_on_board_leaving_station.drop_duplicates(
                subset=["Label", "Boarding time", "Boarding station", "Alighting station"],
                keep=False)  # 在列表中删掉已经下车的乘客

            # print("下车后",i,len(Passenger_num_on_board_leaving_station))
            # print(Passenger_num_on_board_leaving_station)
            if len(Passenger_tobe_proc_per_station[i]) + len(
                    Passenger_num_on_board_leaving_station) <= pn_on_max:  # 若无被滞留的乘客
                ...
                Passenger_num_on_board_leaving_station = pd.concat(
                    [Passenger_num_on_board_leaving_station, Passenger_tobe_proc_per_station[i]])  # 乘客上车
                # print('i=',i,Passenger_num_on_board_leaving_station)

                # print('tobeproc',  Passenger_tobe_proc_per_station[i])
                This_deaparture_pre_wait_time = This_deaparture_pre_wait_time + right_deapart_estimated_arrival_time[
                    i] * len(Passenger_tobe_proc_per_station[i]) - np.sum(
                    Passenger_tobe_proc_per_station[i].iloc[:, 4])  # 计算等待时间
                self.if_depart_wait_time_list[i] += (
                            right_deapart_estimated_arrival_time[i] * len(Passenger_tobe_proc_per_station[i]) - np.sum(
                        Passenger_tobe_proc_per_station[i].iloc[:, 4]))

            else:  # 存在被滞留的乘客
                append_mark = (pn_on_max - len(Passenger_num_on_board_leaving_station))
                Passenger_num_on_board_leaving_station = pd.concat(
                    [Passenger_num_on_board_leaving_station, Passenger_tobe_proc_per_station[i].iloc[:append_mark, :]])
                This_deaparture_pre_wait_time = This_deaparture_pre_wait_time + right_deapart_estimated_arrival_time[
                    i] * append_mark - np.sum(Passenger_tobe_proc_per_station[i].iloc[:append_mark, 4])
                self.if_depart_wait_time_list[i] += (right_deapart_estimated_arrival_time[i] * append_mark - np.sum(
                    Passenger_tobe_proc_per_station[i].iloc[:append_mark, 4]))
                Passenger_cant_takeon_once = Passenger_cant_takeon_once + len(
                    Passenger_tobe_proc_per_station[i]) - append_mark
        ...
        state = torch.cat([torch.Tensor([(right_minute_th // 60) / 24]), torch.Tensor([(right_minute_th % 60) / 60]),
                           torch.Tensor([(np.max(Passenger_num_on_board_leaving_station_num)) / pn_on_max]),
                           torch.Tensor([(This_deaparture_pre_wait_time) / weight_time_wait_time]),
                           torch.Tensor([(np.sum(Passenger_num_on_board_leaving_station_num)) / (
                                       alpha * pn_on_max * (station_num - 1))])])
...
if __name__ == "__main__":
    ...
    for episode in range(max_episode):
        ...
        while True:
            ...
            # else:
            #     beta = 0.2
            reward1 = (XM_2system.Cap_used / (
                        alpha * pn_on_max * (station_num - 1))) - beta * XM_2system.Cant_taken_once  # 原
            reward0 = reward = 1 - (XM_2system.Cap_used / (alpha * pn_on_max * (
            ...
            step += 1
            if (step % 100000 == 0):
                print("episode: ", episode, " step: ", step, 'rightminute: ', right_minute_th, ' reward: ', reward,
                      ' action: ', action, 'bus_on_line_now: ', len(bus_on_line_now), 'reward0: ', reward0,
                      ' reward1: ', reward1)
            state = next_state
            # print("step", step)
            ...
            if XM_2system.end_label == 1:  # 结束一天的标志
                ...
                if avg_flag == 2:
                    adjusted_schedule = adjust_schedule(departure_time, avg_flag=2, start_minute_th=None,
                                                        end_minute_th=None)
                    print("adjusted_schedule", [int(time) for time in adjusted_schedule])  # 最小方差下的发车时刻
                if avg_flag == 3:
                    adjusted_schedule_specific = adjust_schedule(departure_time, avg_flag=3,
                                                                 start_minute_th=start_minute_th,
                                                                 end_minute_th=end_minute_th)
                    print("adjusted_schedule_specific",
                          [int(time) for time in adjusted_schedule_specific])  # 最小方差下的发车时刻
        ...
        if count_departures == count_departures_up:
            break
        # break
        # print(begin - end)
