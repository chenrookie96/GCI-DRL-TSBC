"""
图2-7专用推理脚本
上行：使用shifted数据（晚高峰提前）
下行：使用原始数据（不提前）
展示DRL-TSBC在不对称需求下保持发车次数一致的能力
"""
...
print("="*80)
print("图2-7专用推理：上行shifted + 下行原始")
print("="*80)
print(f"线路: {busline}")
print(f"模型: {model_load_path}")
print(f"上行数据: shifted（晚高峰提前）")
print(f"下行数据: 原始（不提前）")
print("="*80)

# 数据路径 - 上行用shifted，下行用原始
...
if not os.path.exists(passenger_info_path_up):
    print(f"\n错误: 找不到shifted数据: {passenger_info_path_up}")
...
# 输出结果
print("\n" + "="*80)
print("推理结果（图2-7：上行shifted + 下行原始）")
print("="*80)
...
with open(result_file, 'w', encoding='utf-8') as f:
    f.write(f"DRL-TSBC 推理结果（图2-7：上行shifted + 下行原始）\n")
