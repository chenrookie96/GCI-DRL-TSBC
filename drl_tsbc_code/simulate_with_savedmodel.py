...
print("="*80)
print("使用saved_models推理结果进行模拟（原始数据）")
print("="*80)
...
print(f"\n真实需求（原始数据，saved_model） (从7:00开始，共{len(real_need_list_from_7)}个点):")
print([f'{x:.1f}' for x in real_need_list_from_7])

print(f"\nDRL-TSBC容量（原始数据，saved_model） (从7:00开始，共{len(Cap_list_from_7)}个点):")
print([f'{c:.1f}' for c in Cap_list_from_7])

# 保存结果
output_file = f"test_data/{busline}/simulated_demand_capacity_{busline}_savedmodel.txt"
with open(output_file, 'w', encoding='utf-8') as f:
    f.write("真实需求（原始数据，saved_model）:\n")
    f.write(str(real_need_list_from_7) + "\n\n")
    f.write("DRL-TSBC容量（原始数据，saved_model）:\n")
