...
print("="*80)
print("图2-5仿真：增加晚高峰客流前后对比")
print("="*80)

def parse_departure_times(result_file, section_name, direction_name):
    """解析发车时刻表"""
    ...
    if section_name == "调整前":
        ...
        section_pattern = r'【调整后（突发客流）】(.*?)$'
    
    section_match = re.search(section_pattern, content, re.DOTALL)
    if not section_match:
        raise ValueError(f"无法找到{section_name}部分")
...
def simulate_direction(direction, departure_times, use_surge=False, scenario_name=""):
    """模拟单个方向，计算总客运容量和真实需求"""
    direction_name = '上行' if direction == 0 else '下行'
    data_type = '突发客流' if use_surge else '原始'
    
    print(f"\n{'='*80}")
    print(f"模拟: {scenario_name} - {direction_name} - 使用{data_type}数据")
    print(f"{'='*80}")
    
    # 数据路径
    if use_surge and direction == 0:  # 只有上行使用surge数据
        passenger_info_path = f"test_data/{busline}/passenger_dataframe_direction{direction}_surge.csv"
    else:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(result_file):
        ...
        print("请先运行: venv_drl_tsbc\\Scripts\\python.exe drl_tsbc_code/inference_208_surge.py")
        exit(1)
    ...
    # 调整前
    up_times_before = parse_departure_times(result_file, "调整前", "上行")
    down_times_before = parse_departure_times(result_file, "调整前", "下行")
    print(f"调整前: 上行{len(up_times_before)}次, 下行{len(down_times_before)}次")
    
    # 调整后
    up_times_after = parse_departure_times(result_file, "调整后", "上行")
    down_times_after = parse_departure_times(result_file, "调整后", "下行")
    print(f"调整后: 上行{len(up_times_after)}次, 下行{len(down_times_after)}次")
    
    # 仿真调整前（使用原始数据 + 调整前时刻表）
    real_before, cap_before = simulate_direction(0, up_times_before, use_surge=False, scenario_name="调整前")
    
    # 仿真调整后（使用surge数据 + 调整后时刻表）
    real_after, cap_after = simulate_direction(0, up_times_after, use_surge=True, scenario_name="调整后")
    ...
    print(f"\n调整前容量 (共{len(cap_before)}个点):")
    print([f'{x:.1f}' for x in cap_before])
    
    print(f"\n调整前真实需求:")
    print([f'{x:.1f}' for x in real_before])
    
    print(f"\n调整后容量 (共{len(cap_after)}个点):")
    print([f'{x:.1f}' for x in cap_after])
    
    print(f"\n调整后真实需求:")
    print([f'{x:.1f}' for x in real_after])
    
    # 保存结果（转换为普通float）
    output_file = f"test_data/{busline}/figure_2_5_data.txt"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("图2-5数据\n")
        f.write("="*60 + "\n\n")
        f.write("调整前容量:\n")
        f.write(str([float(x) for x in cap_before]) + "\n\n")
        f.write("调整前真实需求:\n")
        f.write(str([float(x) for x in real_before]) + "\n\n")
        f.write("调整后容量:\n")
        f.write(str([float(x) for x in cap_after]) + "\n\n")
        f.write("调整后真实需求:\n")
        f.write(str([float(x) for x in real_after]) + "\n\n")
